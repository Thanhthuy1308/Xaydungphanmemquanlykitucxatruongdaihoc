...
class RoomsPage(ctk.CTkFrame):
    ...
    def render_cards(self, rooms):
        ...
        for offset, room in enumerate(rooms[start:end]):
            ...
            total_beds = room.get("total_beds") or max_people
            occupied_beds = room.get("occupied_beds") or current_people
            ctk.CTkLabel(
                card,
                text=f"Giường: {occupied_beds}/{total_beds}",
                font=ctk.CTkFont(FONT_FAMILY, 11),
                text_color=C["text2"]
...
class RoomDetailDialog(ctk.CTkToplevel):
    ...
    def _build(self):
        ...
        # Bed status is synced from the room capacity and active student assignments.
        bc = ctk.CTkFrame(body, fg_color=C["card"], corner_radius=8,
                          border_width=1, border_color=C["border"])
        bc.grid(row=1,column=0,columnspan=2,sticky="ew",padx=4,pady=4)
        ctk.CTkLabel(bc,text="🛏  Giường trong phòng",
                     font=ctk.CTkFont(FONT_FAMILY,13,"bold"),
                     text_color=C["text"]).pack(anchor="w",padx=14,pady=(10,6))
        from models import database as db
        beds = db.get_room_beds(r['id'])
        bed_grid = ctk.CTkFrame(bc, fg_color="transparent")
        bed_grid.pack(fill="x", padx=10, pady=(0,10))
        for col in range(3):
            bed_grid.grid_columnconfigure(col, weight=1)
        for idx, bed in enumerate(beds):
            occupied = bed.get("status") == "occupied"
            fg = C["rose_bg"] if occupied else C["green_bg"]
            tc = C["rose"] if occupied else C["green"]
            status = bed.get("fullname") or "Trống"
            cell = ctk.CTkFrame(bed_grid, fg_color=fg, corner_radius=8,
                                border_width=1, border_color=C["border"])
            cell.grid(row=idx//3, column=idx%3, sticky="ew", padx=4, pady=4)
            Label(cell, f"Giường {bed.get('bed_number','')}", size=12, bold=True, color=tc).pack(anchor="w", padx=10, pady=(7,0))
            ctk.CTkLabel(cell, text=status, font=ctk.CTkFont(FONT_FAMILY, 11),
                         text_color=C["text2"], anchor="w").pack(anchor="w", padx=10, pady=(0,7))

        # Sinh viên trong phòng
        sc = ctk.CTkFrame(body, fg_color=C["card"], corner_radius=8,
                          border_width=1, border_color=C["border"])
        sc.grid(row=2,column=0,columnspan=2,sticky="ew",padx=4,pady=4)
        ctk.CTkLabel(sc,text="👥  Sinh viên đang ở",
                     font=ctk.CTkFont(FONT_FAMILY,13,"bold"),
                     text_color=C["text"]).pack(anchor="w",padx=14,pady=(10,6))
        occupants = db.get_students_in_room(r['id'])
        if occupants:
            Table(sc,[("Giường",70),("MSSV",120),("Họ và tên",250),("SĐT",130),("Ngày vào",120)],
                  rows=[[s.get('bed_number','—'),s['student_id'],s['fullname'],s.get('phone','—'),s.get('checkin_date','—')]
...
class AddRoomDialogUI(BaseDialog):
    ...
    def _submit(self):
        ...
        except (ValueError, IndexError): _shake(self.ok_btn); return
