...
class BuildingsPage(ctk.CTkFrame):
    ...
    def update_grid(self, buildings):
        ...
        for i, b in enumerate(buildings):
            ...
            manager = b.get("manager_name") or "Chưa phân công"
            MutedLabel(c_card, f"Quản lý: {manager}").pack(anchor="w", padx=14, pady=(0,8))
            ...
            self._bind_detail_clicks(c_card, b)

    def _on_edit(self, b):
        if self.controller: self.controller.show_edit_dialog(b)

    def _bind_detail_clicks(self, widget, building):
        if isinstance(widget, ctk.CTkButton):
            return
        widget.bind("<Button-1>", lambda e, b=building: self._on_detail(b))
        for child in widget.winfo_children():
            self._bind_detail_clicks(child, building)
    ...
    def _on_detail(self, b):
        if self.controller:
            self.controller.show_detail(b)

    def show_detail_dialog(self, detail):
        BuildingDetailDialog(self, detail)


class BuildingDetailDialog(ctk.CTkToplevel):
    """Building detail with manager info, floor groups, rooms, and bed counts."""
    def __init__(self, master, detail):
        super().__init__(master)
        self.detail = detail
        self.title("Chi tiết tòa nhà")
        self.geometry("760x620")
        self.minsize(700, 520)
        self.configure(fg_color=C["bg"])
        self.update_idletasks()
        try:
            x = master.winfo_rootx() + (master.winfo_width() - 760) // 2
            y = master.winfo_rooty() + (master.winfo_height() - 620) // 2
            self.geometry(f"+{x}+{y}")
        except Exception:
            pass
    ...
    def _build(self):
        building = self.detail["building"]
        hdr = ctk.CTkFrame(self, fg_color=C["primary"], corner_radius=0, height=56)
        hdr.pack(fill="x"); hdr.pack_propagate(False)
        ctk.CTkLabel(hdr, text=f"🏢  {building.get('name','')}",
                     font=ctk.CTkFont(FONT_FAMILY, 15, "bold"),
                     text_color=C["white"]).pack(side="left", padx=18, pady=15)
        GhostBtn(hdr, "✕  Đóng", command=self._close, width=90).pack(side="right", padx=14, pady=11)
        ...
        manager = Card(body)
        manager.grid(row=0, column=0, sticky="ew", padx=4, pady=(0,10))
        SectionTitle(manager, "Quản lý tòa nhà").pack(anchor="w", padx=14, pady=(12,6))
        info = ctk.CTkFrame(manager, fg_color="transparent")
        info.pack(fill="x", padx=14, pady=(0,12))
        info.grid_columnconfigure((0,1,2), weight=1)
        items = [
            ("Họ tên", building.get("manager_name") or "Chưa phân công"),
            ("SĐT", building.get("manager_phone") or "—"),
            ("Email", building.get("manager_email") or "—"),
        ]
        for idx, (label, value) in enumerate(items):
            ctk.CTkLabel(info, text=label, font=ctk.CTkFont(FONT_FAMILY, 10, "bold"),
                         text_color=C["text3"]).grid(row=0, column=idx, sticky="w", padx=4)
            ctk.CTkLabel(info, text=value, font=ctk.CTkFont(FONT_FAMILY, 13),
                         text_color=C["text"]).grid(row=1, column=idx, sticky="w", padx=4, pady=(2,0))

        if not self.detail["floors"]:
            MutedLabel(body, "Tòa nhà chưa có phòng").grid(row=1, column=0, pady=24)
            return

        for row_idx, floor in enumerate(self.detail["floors"], start=1):
            fc = Card(body)
            fc.grid(row=row_idx, column=0, sticky="ew", padx=4, pady=6)
            SectionTitle(fc, f"Tầng {floor['floor']}").pack(anchor="w", padx=14, pady=(12,6))
            for room in floor["rooms"]:
                rr = ctk.CTkFrame(fc, fg_color=C["card2"], corner_radius=8)
                rr.pack(fill="x", padx=14, pady=4)
                Label(rr, f"Phòng {room.get('room_number','')}", size=13, bold=True).pack(side="left", padx=10, pady=8)
                bed_text = f"{room.get('occupied_beds') or 0}/{room.get('total_beds') or room.get('capacity') or 0} giường"
                MutedLabel(rr, bed_text).pack(side="left", padx=8)
                MutedLabel(rr, f"{room.get('current_occupants') or 0}/{room.get('capacity') or 0} sinh viên").pack(side="right", padx=10)
...
class AddBuildingDialogUI(BaseDialog):
    def __init__(self, master, submit_callback, prefill=None):
        ...
        super().__init__(master, title, 500, 480)
        ...
        self.field("Quản lý tòa nhà", 4, 0, 2)
        self.manager_name = Entry(self.body, "Nguyễn Văn Quản")
        self.manager_name.grid(row=9, column=0, columnspan=2, sticky="ew", padx=4, pady=2)

        self.field("SĐT quản lý", 5, 0)
        self.manager_phone = Entry(self.body, "09xxxxxxxx")
        self.manager_phone.grid(row=11, column=0, sticky="ew", padx=4, pady=2)

        self.field("Email quản lý", 5, 1)
        self.manager_email = Entry(self.body, "manager@email.com")
        self.manager_email.grid(row=11, column=1, sticky="ew", padx=4, pady=2)
        ...
        if prefill:
            ...
            self.manager_name.delete(0,"end"); self.manager_name.insert(0, prefill.get('manager_name','') or '')
            self.manager_phone.delete(0,"end"); self.manager_phone.insert(0, prefill.get('manager_phone','') or '')
            self.manager_email.delete(0,"end"); self.manager_email.insert(0, prefill.get('manager_email','') or '')

    def _submit(self):
        name = self.name.get().strip()
        if not name: _shake(self.ok_btn); return
        try: floors = int(self.floors.get() or 5)
        except ValueError: floors = 5
        self.submit_callback(
            {"name": name, "floors": floors, "desc": self.desc.get().strip(),
             "manager_name": self.manager_name.get().strip(),
             "manager_phone": self.manager_phone.get().strip(),
             "manager_email": self.manager_email.get().strip()},
            self
        )
