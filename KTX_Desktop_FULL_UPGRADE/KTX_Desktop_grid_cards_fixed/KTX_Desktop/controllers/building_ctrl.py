...
class BuildingController:
    ...
    def _handle_add(self, data, dlg):
        db.add_building(
            data["name"], data["floors"], data["desc"],
            data.get("manager_name", ""), data.get("manager_phone", ""), data.get("manager_email", "")
        )
        dlg._close()
        self.load_data()
    ...
    def _handle_edit(self, data, dlg):
        bid = dlg.prefill['id']
        db.update_building(
            bid, data["name"], data["floors"], data["desc"],
            data.get("manager_name", ""), data.get("manager_phone", ""), data.get("manager_email", "")
        )
        dlg._close()
        self.load_data()

    def show_detail(self, building):
        detail = db.get_building_detail(building["id"])
        if detail:
            self.view.show_detail_dialog(detail)
